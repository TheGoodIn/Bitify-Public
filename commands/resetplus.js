...
var request = require('request');
 ...
 module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(checkguild.docs[0].IsPlus == false){
            const dmero = new Discord.MessageEmbed()
            .setTitle('Error.')
            .setDescription(`You can't reset a server that isn't Bitify+.`)
            .setColor(COLOR)
            .setFooter('Bitify', client.user.avatarURL())
            ...
            const filter2 = async (reaction, user) => {
                return user.id === message.author.id;
                        ...
                        collector11.on('collect', async (reaction, user) => {
                            ...
                            doclookup.update({
                                Role1Type: admin.firestore.FieldValue.delete(),
                                Role1Require: admin.firestore.FieldValue.delete(),
                                Role1ID: admin.firestore.FieldValue.delete(),
                                Role1Status: admin.firestore.FieldValue.delete()
                            ...
                            .setTitle('New Bitify+ Server Reset')
