...
if(check.docs[0] != null){
	var options = {
		...
		body: JSON.stringify({
		  "PublicKeyBase58Check": `${check.docs[0].data().Token}`
		})
	  ...
	  request(options, async function (error, response) {
		...
		const lockedfinal = await formatter.format(locked)
			console.log(check.docs[0].data().NFTLink)
		const embed = new Discord.MessageEmbed()
		.setTitle(`${profilejson.Profile.Username}`)
		.setThumbnail( client.guilds.resolve(message.guild.id).members.resolve(member.id).user.avatarURL())
		.setImage(`https://goodin.link/${check.docs[0].data().NFTLink}`)
	...
	var options = {
	  ...
	  request(options, async function (error, response) {
		...
		var options2 = {
			...
			var optionsnft = {
				'method': 'POST',
				'url': 'https://bitclout.com/api/v0/get-nfts-for-user',
				'headers': {
				  'Content-Type': 'application/json',
				  'Cookie': 'INGRESSCOOKIE=28c0998b1c20ce5b023c12ce4bf159f9'
				},
				body: JSON.stringify({
				  "UserPublicKeyBase58Check": `${profilejson.Profile.PublicKeyBase58Check}`
				})
		  ...
		  request(options2, async function (error2, response2) {
			...
			const followerjson = JSON.parse(response2.body)
			  ...
			  request(optionsnft, async function (error4, response4) {
				if (error4) throw new Error(error4);
				console.log(response4.content)
				const nftjson = JSON.parse(response4.content)

				console.log(nftjson)
				console.log(nftjson.NFTsMap.length)
		...
		const send2 = await message.channel.send(embed)

		if(check.docs[0].data().NFTLink != null){
			console.log('work')
			send2.setImage(check.docs[0].data().NFTLink)
		}
